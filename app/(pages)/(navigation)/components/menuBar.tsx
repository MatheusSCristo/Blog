...
const MenuBar = ({ user }: User) => {
  ...
  return !menuOpen ? (
    <div className="h-fit bg-darkBlue w-16 md:w-36 rounded-xl flex flex-col items-center py-8 relative sticky top-0">
      <div className="sticky top-0 flex items-center flex-col w-full">
        <div className="w-full relative flex items-center">
          <IoIosArrowDropright
            size={30}
            className="absolute right-0 text-white hover:scale-110 hidden sm:block z-10"
    ...
    <div className="hidden sm:flex h-fit bg-white w-[300px] w-1/5 rounded-2xl  flex-col items-center py-8 relative sticky top-0">
      <div className="sticky top-0 flex items-center flex-col">
          ...
          {pages.map((page) => {
            ...
            return (
              <Link
                ...
                className={`${
                  pathname === `/${page.page}`
                    ? "border bg-darkBlue text-white"
                    : ""
                } p-3 rounded-xl hover:scale-105 hover:border w-full text-black hover:bg-darkBlue hover:text-white ease-in duration-200 cursor-pointer flex items-center gap-2`}
              >
                <Icon size={30} />
                <span>{page.name}</span>
              </Link>
